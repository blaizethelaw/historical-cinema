Below is a complete, runnable **Vite + React + TypeScript** project scaffold that wraps the component you just pasted. It avoids heavyweight native binaries and uses **local UI stubs** for `@/components/ui/*` so installs are fast.

---
## File tree
```
historical-cinema-3d/
├─ package.json
├─ tsconfig.json
├─ vite.config.ts
├─ postcss.config.js
...
<html lang="en">
  <head>
    ...
    <title>Historical Cinema 3D</title>
  </head>
  <body class="bg-black">
    <div id="root"></div>
...
body {
  @apply bg-gradient-to-b from-slate-900 via-black to-slate-950 text-white;
  margin: 0;
...
### src/App.tsx
```tsx
import HistoricalCinema3D from "@/components/HistoricalCinema3D";
...
### src/components/HistoricalCinema3D.tsx
```tsx
"use client";
...
---
## How to run
1) Put these files into a folder named `historical-cinema-3d`.
...
If installs hang due to network policy, try `npm config get registry` (expect `https://registry.npmjs.org/`) or run `npm install --no-optional`. This scaffold doesn’t require Next.js, sharp, or Radix so it should install quickly.
